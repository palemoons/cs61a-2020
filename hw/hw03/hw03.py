...
def g_iter(n):
    ...
    if n >= 1 and n <= 3:
        ...
        n1, n2, n3, num = 1, 2, 3, 4
        while num <= n:
            n1, n2, n3, num = n2, n3, n3 + 2 * n2 + 3 * n1, num + 1
        return n3
...
def missing_digits(n):
    ...
    if n < 10:
        ...
        return missing_digits(n // 10) + cal_delta(n % 100)
...
def count_change(total):
    ...
    largest_coin = 2 ** int(math.log2(total))

    return count_partitions(total, largest_coin)
...
def make_anonymous_factorial():
    ...
    # def func(x):
    #     return mul(x, func(sub(x, 1))) if x > 1 else x

    return (lambda f: lambda x: mul(x, f(f)(sub(x, 1))) if x > 1 else x)(
        lambda f: lambda x: mul(x, f(f)(sub(x, 1))) if x > 1 else x
    )
