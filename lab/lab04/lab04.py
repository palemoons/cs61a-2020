...
def reverse_iter(lst):
    ...
    for idx in range(len(lst) // 2):
        lst[idx], lst[len(lst) - idx - 1] = lst[len(lst) - idx - 1], lst[idx]
    return lst
...
def add_chars(w1, w2):
    ...
    if len(w1) < 1:
        ...
        return w2[0 : w2.find(w1[0])] + w2[w2.find(w1[0]) + 1 :]
    else:
        return add_chars(w1[1:], w2[0 : w2.find(w1[0])] + w2[w2.find(w1[0]) + 1 :])
